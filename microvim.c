...
#define MICROVIM_VERSION "1.0.0"
#define MICROVIM_TAB_STOP 4
#define MICROVIM_DEBUG 1
...
struct editorSyntax HLDB[] = {
     ...
     HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS},
     {"Python", Python_HL_extensions, Python_HL_keywords, "#", "\"\"\"!", "\"\"\"!!", // the comments are !""" and """!!
     HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS},
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (MICROVIM_TAB_STOP - 1) - (rx % MICROVIM_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (MICROVIM_TAB_STOP - 1) - (cur_rx % MICROVIM_TAB_STOP);
    cur_rx++;
    if (cur_rx > rx)
      return cx;
  }
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (MICROVIM_TAB_STOP - 1) + 1);
    ...
    if (filerow >= E->numrows) {
      ...
      if (E->numrows == 0 && y == E->screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
                                  "MicroVim text editor - v%s", MICROVIM_VERSION);
